        ...
        <el-col :span="12">
          <el-form-item label="员工头像">
            ...
            <!-- <UploadImg
              ...
              @input="userInfo.staffPhoto = $event"
            /> -->
            <!-- <UploadImg v-model="userInfo.staffPhoto" /> -->
            <!-- <UploadImg
              :url="userInfo.staffPhoto"
              @update:url="userInfo.staffPhoto = $event"
            /> -->
            <UploadImg :url.sync="userInfo.staffPhoto" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
...
export default {
  ...
  async created() {
    ...
    // 获取顶部信息
    const res2 = await getMoreInfo(this.$route.params.id)
    // console.log('res2:', res2)
    // userInfo属性是空  Object.assign,合并属性
    // 动态添加属性给对象 ,vue检测不到
    // Object.assign(this.userInfo, res2)
    // userInfo这个数据本来是存在
    // 直接赋值相当于是覆盖了userInfo vue可以检测到
    this.userInfo = res2
  },
  methods: {
    ...
    async saveUser() {
      ...
      this.$message.success('用户信息更新成功了哦 (づ￣ 3￣)づ!')
      // 重新获取数据
